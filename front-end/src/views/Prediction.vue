            ...
            <div class="form-grid">
              <div class="form-group relative-wrap">
                ...
                <input type="text" v-model="formData.drug_a_name" placeholder="请输入药物A名称" class="form-input"
                  @input="onDrugAInput" @focus="showDrugAList = true" @blur="hideDrugAList" autocomplete="off" />
                <ul class="suggestion-list" v-show="showDrugAList && drugASuggestions.length > 0">
                  <li v-for="(item, index) in drugASuggestions" :key="index" @mousedown="selectDrugA(item)">
              ...
              <div class="form-group relative-wrap">
                ...
                <input type="text" v-model="formData.drug_b_name" placeholder="请输入药物B名称" class="form-input"
                  @input="onDrugBInput" @focus="showDrugBList = true" @blur="hideDrugBList" autocomplete="off" />
                <ul class="suggestion-list" v-show="showDrugBList && drugBSuggestions.length > 0">
                  <li v-for="(item, index) in drugBSuggestions" :key="index" @mousedown="selectDrugB(item)">
                    <span class="main-text">{{ item.name }}</span>
                    <span class="sub-text" v-if="item.smiles" :title="item.smiles">{{ item.smiles.substring(0, 15)
                    }}...</span>
                  </li>
          ...
          <div v-if="predictionResult" class="result-section">
            ...
            <div class="drug-info-grid">
              <div class="drug-card">
                ...
                <div class="drug-details">
                  <p><span class="label">SMILES:</span> <span class="smiles-text"
                      :title="predictionResult.drug_a_info.smiles">{{ predictionResult.drug_a_info.smiles }}</span></p>
              ...
              <div class="drug-card">
                ...
                <div class="drug-details">
                  <p><span class="label">SMILES:</span> <span class="smiles-text"
                      :title="predictionResult.drug_b_info.smiles">{{ predictionResult.drug_b_info.smiles }}</span></p>
            ...
            <div v-if="predictionResult?.attention_analysis" class="attention-section">
              ...
              <div class="model-info">
                <div class="info-item"><span class="label">使用模型：</span><span class="value">{{
                  predictionResult.model_used }}</span></div>
                <div class="info-item"><span class="label">处理时间：</span><span class="value">{{
                  predictionResult.processing_time_ms }} ms</span></div>
                <div class="info-item"><span class="label">时间戳：</span><span class="value">{{
                  formatTimestamp(predictionResult.timestamp) }}</span></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="batch-layout">
          <div class="batch-form-area">
            ...
            <div class="batch-actions">
              ...
              <button class="btn-predict" style="max-width: 100%; margin-top: 12px;" @click="submitBatch"
...
import { standardPredict, batchPredict, searchDdiDrugs } from '../apis/ddis'
...
const showDrugAList = ref(false)
const showDrugBList = ref(false)
const drugASuggestions = ref([])
const drugBSuggestions = ref([])
let timerA = null
let timerB = null
...
const loadingText = ref('🚀 GPU 矩阵运算中...')
...
const submitBatch = async () => {
  ...
  try {
    ...
    if (error.message.includes('timeout') || error.message.includes('网络')) {
      showDialog({
        title: '请求超时',
        message: '前端等待时间过长已断开连接，但后台可能仍在进行计算。请稍后去“历史记录”中查看结果。'
      })
    } else {
...
const onDrugAInput = () => {
  showDrugAList.value = true
  if (timerA) clearTimeout(timerA)
  const keyword = formData.drug_a_name.trim()
  if (!keyword) { drugASuggestions.value = []; return }

  timerA = setTimeout(async () => {
    const res = await searchDdiDrugs(keyword)
    if (res && res.data) drugASuggestions.value = res.data
  }, 300)
}

const onDrugBInput = () => {
  showDrugBList.value = true
  if (timerB) clearTimeout(timerB)
  const keyword = formData.drug_b_name.trim()
  if (!keyword) { drugBSuggestions.value = []; return }

  timerB = setTimeout(async () => {
    const res = await searchDdiDrugs(keyword)
    if (res && res.data) drugBSuggestions.value = res.data
  }, 300)
}

const selectDrugA = (item) => {
  formData.drug_a_name = item.name;
  formData.smiles_a = item.smiles;
  showDrugAList.value = false;
}

const selectDrugB = (item) => {
  formData.drug_b_name = item.name;
  formData.smiles_b = item.smiles;
  showDrugBList.value = false;
}
...
const startLoadingFeedback = () => {
  ...
  progressTimer = setInterval(() => {
    ...
    if (elapsed < 10) {
      loadingText.value = `🚀 GPU 矩阵运算中... (${elapsed}s)`
    } else if (elapsed < 30) {
      loadingText.value = `⏳ 深度特征提取中... (${elapsed}s)`
    } else if (elapsed < 60) {
      loadingText.value = `🔬 模型推理中，请勿关闭页面... (${elapsed}s)`
    } else {
      ...
      if (elapsed === 60) {
        showDialog({
          title: '后台运算中',
          message: '当前批量预测组合较多，GPU 运算耗时较长。您可以继续在此页面等待，或者稍后前往“历史记录”页面查看预测结果。',
  ...
  keepAliveTimer = setInterval(async () => {
    try {
      await getDdiHistory({ page: 1, page_size: 1 })
    } catch (e) {
